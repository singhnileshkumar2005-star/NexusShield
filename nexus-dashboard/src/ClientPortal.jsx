...
export default function ClientPortal({ onBackToAdmin, apiKey = DEFAULT_API_KEY }) {
  ...
  const fetchClientStats = useCallback(async (targetCid) => {
    ...
    try {
      const response = await axios.get(`${HUB_URL}/client-stats/${encodeURIComponent(cid.trim())}`, {
